...
const TYPE_COLOR: Record<string, { bg: string; border: string; text: string }> = {
  ...
  'Meeting':        { bg:'rgba(74,158,255,0.12)', border:'#4A9EFF', text:'#93C5FD' },
  ...
  'Deadline':       { bg:'rgba(239,68,68,0.12)',  border:'#EF4444', text:'#FCA5A5' },
  'Lainnya':        { bg:'rgba(107,115,133,0.12)', border:'#6B7385', text:'#A5AEC0' },
  'Other':          { bg:'rgba(107,115,133,0.12)', border:'#6B7385', text:'#A5AEC0' },
}
    ...
    <div className="max-w-6xl mx-auto animate-fade-in">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3 mb-6">
        <div>
          <h1 className="font-grotesk font-bold text-2xl text-[#EDF0F5] flex items-center gap-3">
            <Calendar size={22} style={{ color:'#4A9EFF' }}/>
            Events & Agenda
          </h1>
          <p className="text-[#6B7385] text-sm mt-1">{upcomingCount} mendatang · {events.length} total event</p>
        </div>
      ...
      <div className="flex flex-wrap gap-3 mb-6 items-center">
        <div style={{ position:'relative', flex:'1', minWidth:'200px', maxWidth:'280px' }}>
          <Search size={14} style={{ position:'absolute', left:'10px', top:'50%', transform:'translateY(-50%)', color:'#6B7385' }}/>
        ...
        <div style={{ display:'flex', gap:'6px', flexWrap:'wrap' }}>
          {['Semua', ...EVENT_TYPES].map(t => {
            ...
            return (
              <button key={t} onClick={() => setTypeFilter(t)} style={{ padding:'5px 12px', borderRadius:'100px', fontSize:'11px', fontWeight:600, cursor:'pointer', border:`1px solid ${active ? (c?.border ?? '#FF6A1A') : 'rgba(255,255,255,0.07)'}`, background: active ? (c?.bg ?? 'rgba(255,106,26,0.12)') : 'transparent', color: active ? (c?.text ?? '#FF8A4C') : '#6B7385' }}>
        ...
        <select value={divFilter} onChange={e => setDivFilter(e.target.value)} style={{ background:'#141925', border:'1px solid rgba(255,255,255,0.08)', borderRadius:'10px', padding:'8px 12px', color:'#EDF0F5', fontSize:'12px', outline:'none', cursor:'pointer' }}>
        ...
        <div style={{ display:'flex', background:'#141925', border:'1px solid rgba(255,255,255,0.08)', borderRadius:'10px', padding:'3px', gap:'2px' }}>
          {([['all','Semua'],['upcoming','Mendatang'],['past','Lalu']] as const).map(([k,l]) => (
            <button key={k} onClick={() => setTimeFilter(k)} style={{ padding:'5px 10px', borderRadius:'7px', border:'none', cursor:'pointer', fontSize:'11px', fontWeight:700, background: timeFilter===k ? '#4A9EFF' : 'transparent', color: timeFilter===k ? '#0C0F16' : '#6B7385', fontFamily:"'Space Grotesk',sans-serif" }}>{l}</button>
          ))}
        </div>
      </div>
        ...
        <div>
          {filtered.length === 0 ? (
            <div style={{ background:'#10141d', border:'1px dashed rgba(255,255,255,0.1)', borderRadius:'16px', padding:'60px 24px', textAlign:'center' }}>
              <Calendar size={36} style={{ color:'#4a5160', margin:'0 auto 12px' }}/>
              <p style={{ color:'#6B7385', fontSize:'14px' }}>Tidak ada event ditemukan</p>
            </div>
          ) : (
                    ...
                    {grouped.get(key)!.map(ev => {
                      ...
                      return (
                        <div key={ev.id} onClick={() => setSelected(isSelected ? null : ev)}
                          style={{ display:'grid', gridTemplateColumns:'56px 1fr auto', gap:'12px', background: isSelected ? `${c.bg}` : '#10141d', border:`1px solid ${isSelected ? c.border+'60' : 'rgba(255,255,255,0.07)'}`, borderRadius:'14px', padding:'14px 16px', cursor:'pointer', opacity: past ? 0.7 : 1, transition:'all 0.15s', alignItems:'center' }}
                          onMouseEnter={e => !isSelected && (e.currentTarget.style.borderColor='rgba(255,255,255,0.14)')}
                          onMouseLeave={e => !isSelected && (e.currentTarget.style.borderColor='rgba(255,255,255,0.07)')}>

                          <div style={{ textAlign:'center', background:'rgba(255,255,255,0.04)', borderRadius:'10px', padding:'8px 4px' }}>
                            <p style={{ fontSize:'20px', fontWeight:800, color:'#EDF0F5', fontFamily:"'Space Grotesk',sans-serif", lineHeight:1 }}>
                              {new Date(ev.startAt).getDate()}
                            </p>
                            <p style={{ fontSize:'10px', color:'#6B7385', fontFamily:"'IBM Plex Mono',monospace", textTransform:'uppercase' }}>
                          ...
                          <div style={{ minWidth:0 }}>
                            <div style={{ display:'flex', alignItems:'center', gap:'8px', marginBottom:'3px' }}>
                              ...
                              <p style={{ fontSize:'14px', fontWeight:700, color:'#EDF0F5', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{ev.title}</p>
                            </div>
                            {ev.eventName && (
                              <p style={{ fontSize:'11px', color:'#A5AEC0', marginBottom:'4px', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', fontStyle:'italic' }}>"{ev.eventName}"</p>
                            )}
                            <div style={{ display:'flex', flexWrap:'wrap', gap:'6px', alignItems:'center' }}>
                              ...
                              <span style={{ fontSize:'11px', color:'#6B7385', display:'flex', alignItems:'center', gap:'3px' }}><Clock size={10}/>{ev.allDay ? 'Seharian' : fmtTime(ev.startAt)}</span>
                              {ev.location && <span style={{ fontSize:'11px', color:'#6B7385', display:'flex', alignItems:'center', gap:'3px' }}><MapPin size={10}/>{ev.location}</span>}
                              {evParticipants.length > 0 && <span style={{ fontSize:'11px', color:'#6B7385', display:'flex', alignItems:'center', gap:'3px' }}><Users size={10}/>{evParticipants.length} orang</span>}
                            </div>
                          </div>

                          <ChevronRight size={14} style={{ color: isSelected ? c.border : '#4a5160', transform: isSelected ? 'rotate(90deg)' : 'none', transition:'transform 0.2s', flexShrink:0 }}/>
                        </div>
                      )
                    })}
                  </div>
            ...
            <div style={{ background:'#10141d', border:`1px solid ${c.border}40`, borderRadius:'18px', overflow:'hidden', position:'sticky', top:'80px', maxHeight:'calc(100vh - 120px)', overflowY:'auto' }}>
              ...
              <div style={{ padding:'16px 20px', borderBottom:'1px solid rgba(255,255,255,0.07)', display:'flex', justifyContent:'space-between', alignItems:'center', background:c.bg, position:'sticky', top:0, zIndex:1 }}>
                ...
                <div style={{ display:'flex', gap:'6px' }}>
                  {isOwner && <button onClick={() => openEdit(selected)} style={{ background:'none', border:'none', cursor:'pointer', color:'#6B7385', display:'flex', padding:'4px' }}><Pencil size={14}/></button>}
                  ...
                  <button onClick={() => setSelected(null)} style={{ background:'none', border:'none', cursor:'pointer', color:'#6B7385', display:'flex', padding:'4px' }}><X size={16}/></button>
                </div>
              </div>

              <div style={{ padding:'20px' }}>
                <h2 style={{ fontSize:'18px', fontWeight:800, color:'#EDF0F5', fontFamily:"'Space Grotesk',sans-serif", marginBottom:'4px', lineHeight:1.3 }}>{selected.title}</h2>
                {selected.eventName && (
                  <p style={{ fontSize:'13px', color:'#A5AEC0', marginBottom:'16px', fontStyle:'italic' }}>Nama Acara: "{selected.eventName}"</p>
                )}
                ...
                {selected.notes && (
                  <div style={{ background:'rgba(255,255,255,0.03)', borderRadius:'10px', padding:'12px', marginBottom:'16px' }}>
                    <p style={{ fontSize:'12px', color:'#A5AEC0', lineHeight:1.7, whiteSpace:'pre-wrap' }}>{selected.notes}</p>
                  </div>
                )}
                  ...
                  <div style={{ marginBottom:'16px' }}>
                    ...
                    <div style={{ display:'flex', flexDirection:'column', gap:'6px' }}>
                      {evParticipants.map(p => (
                        <div key={p.userId} style={{ display:'flex', alignItems:'center', gap:'10px', background:'rgba(255,255,255,0.03)', borderRadius:'9px', padding:'8px 12px' }}>
                          ...
                          <div style={{ flex:1, minWidth:0 }}>
                            <p style={{ fontSize:'12px', fontWeight:700, color:'#EDF0F5', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{p.userName ?? '—'}</p>
                ...
                {past && (
                  <div style={{ background:'rgba(107,115,133,0.1)', border:'1px solid rgba(107,115,133,0.2)', borderRadius:'9px', padding:'8px 12px', marginBottom:'14px' }}>
                    <p style={{ fontSize:'11px', color:'#6B7385' }}>Event ini sudah berlalu</p>
                  </div>
                )}
                ...
                <div style={{ display:'flex', gap:'8px', flexWrap:'wrap' }}>
                  ...
                  <Link href="/calendar"
                    style={{ display:'flex', alignItems:'center', gap:'6px', background:'rgba(255,255,255,0.05)', border:'1px solid rgba(255,255,255,0.1)', borderRadius:'9px', padding:'8px 14px', color:'#A5AEC0', fontSize:'12px', fontWeight:600, textDecoration:'none', fontFamily:"'Space Grotesk',sans-serif" }}>
...
function MetaItem({ icon, label, children }: { icon: React.ReactNode; label: string; children: React.ReactNode }) {
  return (
    <div style={{ display:'flex', gap:'8px', alignItems:'flex-start' }}>
      <div style={{ color:'#6B7385', paddingTop:'1px', flexShrink:0 }}>{icon}</div>
      <div>
        <p style={{ fontSize:'10px', color:'#4a5160', fontFamily:"'IBM Plex Mono',monospace", letterSpacing:'0.04em', textTransform:'uppercase', marginBottom:'1px' }}>{label}</p>
        <p style={{ fontSize:'13px', color:'#A5AEC0' }}>{children}</p>
      </div>
    </div>
  )
}
...
function EventModal({ mode, event, existingParticipants, divisions, allUsers, currentUser, onClose, onSaved }:
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto flex items-start justify-center p-4 py-8" style={{ background:'rgba(0,0,0,0.75)' }}>
      <div className="w-full max-w-2xl rounded-2xl flex flex-col max-h-[90vh]" style={{ background:'#10141d', border:'1px solid rgba(255,255,255,0.1)' }}>
        {/* Modal header */}
        <div style={{ padding:'16px 24px', borderBottom:'1px solid rgba(255,255,255,0.07)', display:'flex', justifyContent:'space-between', alignItems:'center', background:'#10141d', flexShrink:0 }}>
          <h2 className="font-grotesk font-bold text-[16px] text-[#EDF0F5]">{mode==='create'?'Buat Event Baru':'Edit Event'}</h2>
        ...
        <form onSubmit={submit} style={{ display:'flex', flexDirection:'column', flex:1, overflow:'hidden' }}>
          <div style={{ padding:'20px 24px', display:'flex', flexDirection:'column', gap:'14px', overflowY:'auto', flex:1 }}>
                ...
                {EVENT_TYPES.map(t => {
                  ...
                  return (
                    <button key={t} type="button" onClick={() => setForm(f=>({...f,eventType:t}))}
                      style={{ padding:'6px 12px', borderRadius:'100px', fontSize:'11px', fontWeight:600, cursor:'pointer', border:`1px solid ${active ? c.border : 'rgba(255,255,255,0.07)'}`, background: active ? c.bg : 'transparent', color: active ? c.text : '#6B7385', transition:'all 0.12s' }}>
                      {TYPE_ICONS[t]} {t}
                    </button>
                  )
                })}
            ...
            <div style={{ display:'flex', alignItems:'center', gap:'10px' }}>
              <input type="checkbox" id="allDay" checked={form.allDay} onChange={e => setForm(f=>({...f,allDay:e.target.checked}))} style={{ accentColor:'#4A9EFF', width:'16px', height:'16px' }}/>
              <label htmlFor="allDay" style={{ fontSize:'13px', color:'#A5AEC0', cursor:'pointer' }}>Seharian (All Day)</label>
            </div>
            ...
            <div style={{ borderTop:'1px solid rgba(255,255,255,0.06)', paddingTop:'14px' }}>
              ...
              {participants.length > 0 && (
                <div style={{ display:'flex', flexDirection:'column', gap:'6px', marginBottom:'10px' }}>
                  {participants.map(p => (
                    <div key={p.userId} style={{ display:'grid', gridTemplateColumns:'1fr 1fr auto', gap:'8px', alignItems:'center', background:'rgba(255,255,255,0.03)', borderRadius:'9px', padding:'8px 10px' }}>
                      <p style={{ fontSize:'12px', fontWeight:600, color:'#EDF0F5', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{getUserName(p.userId)}</p>
                      <input
                        style={{ background:'rgba(255,255,255,0.05)', border:'1px solid rgba(255,255,255,0.1)', borderRadius:'7px', padding:'5px 8px', color:'#EDF0F5', fontSize:'12px', outline:'none', fontFamily:"'Plus Jakarta Sans',sans-serif" }}
              ...
              <div style={{ position:'relative' }}>
                <Search size={13} style={{ position:'absolute', left:'10px', top:'50%', transform:'translateY(-50%)', color:'#6B7385' }}/>
                <input
                ...
                {userSearch && availableUsers.length > 0 && (
                  <div style={{ position:'absolute', top:'100%', left:0, right:0, background:'#141925', border:'1px solid rgba(255,255,255,0.1)', borderRadius:'10px', zIndex:10, overflow:'hidden', marginTop:'4px', maxHeight:'180px', overflowY:'auto' }}>
                    {availableUsers.slice(0,8).map(u => (
                      <button key={u.id} type="button" onClick={() => addParticipant(u.id)}
                        style={{ display:'flex', alignItems:'center', gap:'10px', width:'100%', padding:'10px 14px', background:'none', border:'none', cursor:'pointer', color:'#EDF0F5', textAlign:'left' }}
                        ...
                        <div>
                          ...
                          <p style={{ fontSize:'10px', color:'#6B7385' }}>Staff</p>
                        </div>
              ...
              {participants.length > 0 && (
                <p style={{ fontSize:'11px', color:'#4a5160', marginTop:'6px' }}>{participants.length} orang di-assign ke event ini</p>
              )}
            </div>
          </div>

          {/* Footer */}
          <div style={{ padding:'16px 24px', borderTop:'1px solid rgba(255,255,255,0.07)', display:'flex', gap:'10px', background:'#10141d', flexShrink:0 }}>
            ...
            <button type="submit" disabled={loading} style={{ flex:2, background:loading?'rgba(74,158,255,0.4)':'rgba(74,158,255,0.9)', border:'none', color:'#0C0F16', borderRadius:'10px', padding:'11px', cursor:'pointer', fontFamily:"'Space Grotesk',sans-serif", fontWeight:700, fontSize:'14px' }}>
