...
// Role "di atas arena" (tak ikut leaderboard/EXP) → kartu prestige bertema "Game Master".
const PRESTIGE_META: Record<string, { icon: string; title: string; tagline: string }> = {
  super_admin:   { icon: '⚙️', title: 'THE ARCHITECT',  tagline: 'Perancang & penjaga sistem Terminal GODA' },
  head_director: { icon: '👑', title: 'SANG VISIONER',  tagline: 'Nahkoda Department Terminal GODA' },
  spv_manager:   { icon: '🎖️', title: 'THE STRATEGIST', tagline: 'Panglima operasional lintas divisi' },
}

export function CharacterCard({ user, expBySource = {}, mood, prestige = false, onClose }: {
  user: CharacterUser
  ...
  const accent = prestige ? roleColor : tierColor   // warna utama kartu
  const earned = user.badges.filter(b => b.badgeId)
  const sources = Object.entries(expBySource).filter(([, v]) => v > 0).sort((a, b) => b[1] - a[1])
  const maxSource = Math.max(1, ...sources.map(([, v]) => v))
  const pm = PRESTIGE_META[user.role] ?? { icon: '✨', title: 'GAME MASTER', tagline: 'Di luar arena' }
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto flex items-start justify-center p-4 py-8"
      ...
      <div className="w-full max-w-[420px] rounded-2xl overflow-hidden animate-fade-in"
        style={{ background: 'var(--bg-elevated)', border: `1px solid ${accent}55`, boxShadow: `0 24px 60px -20px ${accent}66` }}
        onClick={e => e.stopPropagation()}>

        {/* ── HERO ── */}
        <div style={{ position: 'relative', padding: '26px 24px 20px', textAlign: 'center',
          background: `radial-gradient(120% 130% at 50% -10%, ${accent}33 0%, transparent 60%), var(--bg-card)` }}>
          ...
          {/* Ribbon: rank (pemain) atau "GAME MASTER" (prestige) */}
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: 11, fontWeight: 800, color: accent, background: `${accent}1F`, border: `1px solid ${accent}55`, borderRadius: 100, padding: '3px 11px', marginBottom: 14, fontFamily: "'IBM Plex Mono',monospace" }}>
            {prestige ? `${pm.icon} GAME MASTER` : `${user.rank && user.rank <= 3 ? MEDAL[user.rank] : ''} PERINGKAT #${user.rank ?? '—'}`}
          </div>

          {/* Avatar + ring */}
          <div style={{ width: 96, height: 96, margin: '0 auto', borderRadius: '50%', padding: 3,
            background: `conic-gradient(from 180deg, ${accent}, #FF6A1A, ${accent})`, boxShadow: `0 0 26px ${accent}77` }}>
            <div style={{ width: '100%', height: '100%', borderRadius: '50%', overflow: 'hidden', background: 'var(--bg-elevated)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              {user.avatarUrl
                ...
                : <span style={{ fontSize: 34, fontWeight: 800, color: accent, fontFamily: "'Space Grotesk',sans-serif" }}>{user.fullName.charAt(0)}</span>}
            </div>
          </div>

          {/* Badge overlap: LV/tier (pemain) atau title (prestige) */}
          <div style={{ marginTop: -16, display: 'inline-flex', alignItems: 'center', gap: 6, background: accent, color: '#0C0F16', borderRadius: 100, padding: '3px 12px', fontWeight: 800, fontSize: 12, fontFamily: "'Space Grotesk',sans-serif", border: '2px solid var(--bg-elevated)' }}>
            {prestige ? `${pm.icon} ${pm.title}` : `LV ${lvl.level} · ${lvl.title}`}
          </div>
        ...
        {prestige ? (
          /* ── PRESTIGE BODY (Direktur/Manager/Super Admin) ── */
          <div style={{ padding: '18px 24px 24px', borderTop: '1px solid var(--border)' }}>
            <div style={{ background: `radial-gradient(120% 140% at 0% 0%, ${accent}14 0%, transparent 55%), var(--surface-subtle)`, border: `1px solid ${accent}33`, borderRadius: 14, padding: '16px 16px', textAlign: 'center' }}>
              <div style={{ fontSize: 30, marginBottom: 6 }}>{pm.icon}</div>
              <div style={{ fontSize: 13.5, fontWeight: 800, color: 'var(--text-primary)', fontFamily: "'Space Grotesk',sans-serif", letterSpacing: '0.04em' }}>{pm.title}</div>
              <div style={{ fontSize: 12, color: 'var(--text-secondary)', marginTop: 4, lineHeight: 1.5 }}>{pm.tagline}</div>
            </div>
            <p style={{ fontSize: 11.5, color: 'var(--text-muted)', textAlign: 'center', marginTop: 12, lineHeight: 1.5 }}>
              🎯 Bukan pemain di papan EXP — peran di atas arena: mengawasi, menilai, & membimbing tim.
            </p>
          </div>
        ) : (
          <>
