...
const ACTION_COLORS: Record<string, string> = {
  pending: 'var(--text-muted)', approve: 'var(--green)', reject: 'var(--red)', revision: 'var(--peach)',
}
      ...
      <div style={{ width: '100%', maxWidth: '440px', background: 'var(--bg-elevated)', borderLeft: '1px solid var(--border)', height: '100%', overflowY: 'auto', display: 'flex', flexDirection: 'column' }}
        ...
        <div style={{ padding: '20px 24px 14px', borderBottom: '1px solid var(--border)', flexShrink: 0 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '12px' }}>
            <div style={{ flex: 1 }}>
              <h2 style={{ fontSize: '15px', fontWeight: 700, color: 'var(--text-primary)', lineHeight: 1.35, fontFamily: "'Space Grotesk', sans-serif" }}>{task.name}</h2>
            </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '16px 24px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
            ...
            <div style={{ background: 'var(--surface-subtle)', borderRadius: '10px', padding: '10px 12px' }}>
              ...
              <p style={{ fontSize: '12px', color: task.isOverdue ? 'var(--red)' : 'var(--text-secondary)', fontFamily: "'IBM Plex Mono', monospace" }}>
              ...
              {task.assignees.length > 0 ? (
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '3px' }}>
                  {task.assignees.map(a => (
                    <span key={a.id} style={{ fontSize: '11px', color: 'var(--text-secondary)', background: 'var(--border)', borderRadius: '5px', padding: '2px 6px', display: 'flex', alignItems: 'center', gap: '4px' }}>
                      <AvatarChipSmall name={a.fullName} url={a.avatarUrl} />
                      {a.fullName}
                    </span>
                  ))}
                </div>
              ) : <p style={{ fontSize: '12px', color: 'var(--text-faint)' }}>—</p>}
            </div>
          </div>
          ...
          {task.description && (
            <div style={{ background: 'var(--surface-subtle)', borderRadius: '10px', padding: '12px' }}>
              ...
              <p style={{ fontSize: '13px', color: 'var(--text-secondary)', lineHeight: 1.6, whiteSpace: 'pre-wrap' }}>{renderWithLinks(task.description)}</p>
            </div>
          )}
                ...
                {logs.map(log => (
                  <div key={log.id} style={{ background: 'var(--surface-subtle)', border: '1px solid var(--border)', borderRadius: '10px', padding: '10px 12px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '5px' }}>
                      ...
                      <span style={{ fontSize: '10px', color: 'var(--text-faint)', fontFamily: "'IBM Plex Mono', monospace", marginLeft: 'auto' }}>{fmtLog(log.created_at)}</span>
                    </div>
                    <p style={{ fontSize: '13px', color: 'var(--text-secondary)', lineHeight: 1.5, whiteSpace: 'pre-wrap' }}>{renderWithLinks(log.note)}</p>
                  </div>
                ))}
              </div>
            )}
...
export function ProjectDetailContent({ project, currentUserRole, currentUserId, comments = [], projectMembers = [], allUsers = [], currentUser }: Props) {
  ...
  return (
    <div className="space-y-5 max-w-5xl mx-auto animate-fade-in">
      {/* Back + header */}
      <div>
        <Link href="/projects" className="inline-flex items-center gap-1.5 text-[var(--text-muted)] hover:text-[var(--text-secondary)] text-[13px] mb-4 transition-colors">
          <ArrowLeft size={14} /> Kembali ke Projects
        </Link>
        <div className="flex flex-col sm:flex-row sm:items-start gap-4">
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-1">
              <span className="font-mono text-[10px] text-[var(--text-muted)] tracking-widest">{project.projectCode}</span>
              {project.isOverdue && <Badge variant="red">Overdue</Badge>}
            </div>
            <h1 className="font-grotesk font-bold text-2xl text-[var(--text-primary)] leading-tight">{project.name}</h1>
            <div className="flex items-center gap-3 mt-2 flex-wrap">
              ...
              {isLeaderPlus ? (
                <select
                  ...
                  style={{
                    ...
                    borderRadius: '8px', padding: '3px 10px', fontSize: '12px', color: 'var(--text-primary)',
              ...
              {(project.approval?.status === 'Approved' || isCompleted) && (
                <span style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', background: 'rgba(63,208,138,0.12)', border: '1px solid rgba(63,208,138,0.35)', borderRadius: '100px', padding: '2px 10px', fontSize: '11px', fontWeight: 700, color: 'var(--green)', fontFamily: "'Space Grotesk', sans-serif" }}>
          ...
          <Link href={`/projects/${project.id}/edit`}
            ...
            style={{ background: 'var(--border)', border: '1px solid var(--border-strong)', color: 'var(--text-secondary)' }}>
            Edit
          </Link>
        </div>
      </div>
      ...
      {underReview && (
        <div className="flex items-center gap-2.5 px-4 py-3 rounded-xl"
          ...
          <span className="text-[13px] text-[var(--peach)]">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <Card>
            <CardBody>
              <div className="flex items-center justify-between mb-2">
                <span className="font-grotesk font-semibold text-[14px] text-[var(--text-primary)]">Progress</span>
              ...
              <p className="text-[12px] text-[var(--text-muted)] mt-2">
                {total > 0
                  ? <><span style={{ color: 'var(--green)', fontWeight: 600 }}>{done}</span> dari {total} task berstatus Completed</>
          ...
          <Card>
            <CardBody className="space-y-4">
              <div>
                <div className="font-mono text-[10px] text-[var(--text-muted)] tracking-widest uppercase mb-2">Objective</div>
                <p className="text-[13.5px] text-[var(--text-secondary)] leading-relaxed">{project.objective}</p>
              </div>
              <div>
                <div className="font-mono text-[10px] text-[var(--text-muted)] tracking-widest uppercase mb-2">Deliverables</div>
                <p className="text-[13.5px] text-[var(--text-secondary)] leading-relaxed">{project.deliverables}</p>
              </div>
              {project.notes && (
                <div>
                  <div className="font-mono text-[10px] text-[var(--text-muted)] tracking-widest uppercase mb-2">Catatan</div>
                  <p className="text-[13.5px] text-[var(--text-secondary)] leading-relaxed">{project.notes}</p>
                </div>
              )}
              {project.attachmentUrl && (
                <a href={project.attachmentUrl} target="_blank" rel="noopener noreferrer"
                  className="inline-flex items-center gap-1.5 text-[var(--blue)] text-[13px] hover:underline">
                  📎 Lihat lampiran
                </a>
              )}
            </CardBody>
          ...
          <Card>
            <div className="flex items-center justify-between p-4 border-b border-[var(--border)]">
              <span className="font-grotesk font-semibold text-[14px] text-[var(--text-primary)]">
                Tasks <span className="text-[var(--text-muted)] font-normal">({done}/{total} Completed)</span>
              </span>
            ...
            <div className="divide-y divide-[var(--surface-hover)]">
              {tasks.length === 0 ? (
                ...
                return (
                  <div key={t.id} className="flex items-center gap-3 p-3.5 cursor-pointer hover:bg-[var(--surface-hover)] transition-colors rounded-lg"
                    ...
                    <button onClick={e => { e.stopPropagation(); if (editable && busyTaskId !== t.id) toggleCheck(t) }} disabled={!editable || busyTaskId === t.id}
                      ...
                      {t.checked && <Check size={13} strokeWidth={3} style={{ color: 'var(--on-accent)' }} />}
                    </button>
                    <div className="flex-1 min-w-0">
                      <div className="text-[13px] text-[var(--text-primary)] truncate" style={{ textDecoration: t.checked ? 'line-through' : 'none', opacity: t.checked ? 0.6 : 1 }}>
                        {t.name}
                      </div>
            ...
            <Card>
              <div className="p-4 border-b border-[var(--border)]">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-grotesk font-semibold text-[14px] text-[var(--text-primary)]">Review Hasil — Berjenjang</span>
                </div>
              </div>
                ...
                {[1, 2, 3].map(step => {
                  ...
                  return (
                    <div key={step} className="flex items-center gap-4 p-3 rounded-xl"
                      style={{ background: isActive ? 'rgba(255,106,26,0.08)' : 'var(--surface-subtle)', border: `1px solid ${isActive ? 'rgba(255,106,26,0.25)' : 'var(--surface-hover)'}` }}>
                      ...
                      <div className="flex-1">
                        <div className="text-[13px] text-[var(--text-primary)] font-medium">{STEP_LABELS[step]}</div>
                        {s?.note && <div className="text-[11px] text-[var(--text-secondary)] mt-0.5">"{s.note}"</div>}
                      </div>
        ...
        <div className="space-y-4">
          <Card>
            <CardBody className="space-y-4">
              <div>
                <div className="font-mono text-[10px] text-[var(--text-muted)] tracking-widest uppercase mb-1.5 flex items-center gap-1.5">
                  <Users size={11} /> PIC
                </div>
                <div className="flex items-center gap-2">
                  <Avatar name={project.picName ?? '?'} size="sm" imageUrl={project.picAvatar} />
                  <span className="text-[13px] text-[var(--text-primary)]">{project.picName ?? '—'}</span>
                </div>
              </div>

              {project.members?.length > 0 && (
                <div>
                  <div className="font-mono text-[10px] text-[var(--text-muted)] tracking-widest uppercase mb-1.5">Tim ({project.members.length})</div>
                  <div className="flex flex-wrap gap-1.5">
                    {project.members.slice(0, 8).map(m => m.fullName && (
                      <div key={m.userId} className="flex items-center gap-1.5 px-2 py-1 rounded-lg" style={{ background: 'var(--surface-hover)' }}>
                        <Avatar name={m.fullName} size="xs" imageUrl={m.avatarUrl} />
                        <span className="text-[11px] text-[var(--text-secondary)]">{m.fullName.split(' ')[0]}</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div>
                ...
                <div className="text-[12px] text-[var(--text-secondary)] space-y-0.5">
                  ...
                  <div>Deadline: <span style={{ color: project.isOverdue ? 'var(--red)' : 'var(--text-primary)' }}>
          ...
          {project.budgetPlanned !== null && canViewBudget(currentUserRole) && (
            <Card>
              <CardBody className="space-y-3">
                <div className="font-mono text-[10px] text-[var(--text-muted)] tracking-widest uppercase flex items-center gap-1.5 mb-1">
                ...
                {[
                  ...
                  <div key={label} className="flex justify-between items-center">
                    ...
                    <span className="font-mono text-[12px]"
                      style={{ color: label === 'Remaining' && budgetRemaining < 0 ? 'var(--red)' : 'var(--text-primary)' }}>
