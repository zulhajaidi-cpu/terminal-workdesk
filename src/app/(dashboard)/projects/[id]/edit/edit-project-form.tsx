  ...
  return (
    <div className="max-w-4xl mx-auto animate-fade-in pb-16">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="font-grotesk font-bold text-2xl text-[#EDF0F5]">Edit Project</h1>
          <p style={{ fontSize: '12px', color: '#6B7385', marginTop: '2px', fontFamily: "'IBM Plex Mono', monospace" }}>{project.projectCode}</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div style={sectionStyle}>
          <div className="flex items-center justify-between mb-1">
            <div>
              <h2 className="font-grotesk font-bold text-[15px] text-[#EDF0F5]">
                Tasks Project
                {totalTaskCount > 0 && (
                  <span style={{ fontSize: '12px', color: '#6B7385', fontWeight: 400, marginLeft: '8px' }}>({totalTaskCount} task)</span>
                )}
              </h2>
          ...
          <div className="space-y-3 mt-3">
            ...
            {existingTasks.map((task, idx) => (
              <div key={task.id} style={{ background: '#141925', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '12px', padding: '16px' }}>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span style={{ fontSize: '11px', color: '#3FD08A', background: 'rgba(63,208,138,0.08)', border: '1px solid rgba(63,208,138,0.2)', padding: '1px 8px', borderRadius: '6px' }}>tersimpan</span>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  <div className="md:col-span-1">
                    ...
                    <div style={{ ...inputStyle, color: '#A5AEC0', cursor: 'default' }}>{task.name}</div>
                  </div>
                  <div>
                    <label style={{ ...labelStyle, marginBottom: '4px' }}>Due Date</label>
                    <div style={{ ...inputStyle, color: '#A5AEC0', cursor: 'default' }}>{task.dueDate || '—'}</div>
                  </div>
                  <div>
                    <label style={{ ...labelStyle, marginBottom: '4px' }}>Prioritas</label>
                    <div style={{ ...inputStyle, color: '#A5AEC0', cursor: 'default' }}>{task.priority}</div>
                  </div>
                </div>
                {task.assignees.length > 0 && (
                  <div className="flex flex-wrap gap-2 mt-3">
                    {task.assignees.map(a => (
                      <span key={a.id} style={{ fontSize: '12px', background: 'rgba(74,158,255,0.08)', border: '1px solid rgba(74,158,255,0.2)', color: '#4A9EFF', padding: '2px 10px', borderRadius: '100px' }}>
                        {a.fullName}
                      </span>
                    ))}
                  </div>
            ...
            {newTaskDrafts.map((task, idx) => (
              <div key={task.id} style={{ background: '#141925', border: '1px solid rgba(255,106,26,0.2)', borderRadius: '12px', padding: '16px' }}>
                <div className="flex items-center gap-2 mb-3">
                  <span style={{ fontSize: '11px', color: '#6B7385', fontFamily: "'IBM Plex Mono', monospace", background: 'rgba(255,255,255,0.05)', padding: '2px 8px', borderRadius: '6px' }}>
                    Task baru {idx + 1}
                  </span>
        ...
        <div className="flex gap-3">
          ...
          <button type="submit" disabled={loading}
            style={{ flex: 2, background: loading ? 'rgba(255,106,26,0.5)' : '#FF6A1A', border: 'none', color: '#0C0F16', borderRadius: '11px', padding: '12px', cursor: loading ? 'not-allowed' : 'pointer', fontFamily: "'Space Grotesk', sans-serif", fontWeight: 700, fontSize: '14px' }}>
