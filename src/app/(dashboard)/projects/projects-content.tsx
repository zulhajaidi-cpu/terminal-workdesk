  ...
  return (
    <div className="space-y-5 max-w-7xl mx-auto animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="font-grotesk font-bold text-2xl text-[#EDF0F5]">Projects</h1>
          <p className="text-[#6B7385] text-sm mt-0.5">
            {activeCount} aktif · <span style={{ color: '#FF6B6B' }}>{overdueCount} overdue</span>
          </p>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {canCreateProject(currentUser.role) && (
            <Link href="/projects/new"
              className="flex items-center gap-2 px-4 py-2.5 rounded-xl font-grotesk font-semibold text-[13px] text-[#0C0F16] transition-all"
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <select value={divisionFilter} onChange={e => setDivisionFilter(e.target.value)}
          className="px-3 py-2.5 rounded-xl text-[13px] text-[#EDF0F5] outline-none cursor-pointer"
          style={{ background: '#141925', border: '1px solid rgba(255,255,255,0.07)' }}>
          <option>Semua</option>
      ...
      <div className="flex gap-2 overflow-x-auto pb-1 hide-scroll">
        {STATUS_FILTERS.map(s => (
          <button key={s} onClick={() => setStatusFilter(s)} className="flex-shrink-0 px-3 py-1.5 rounded-lg text-[12px] font-medium transition-all"
            style={{ background: statusFilter === s ? 'rgba(255,106,26,0.15)' : 'rgba(255,255,255,0.04)', border: `1px solid ${statusFilter === s ? 'rgba(255,106,26,0.4)' : 'rgba(255,255,255,0.07)'}`, color: statusFilter === s ? '#FF8A4C' : '#A5AEC0' }}>
            {s}
          </button>
        ))}
      </div>
      ...
      {isSuperAdmin && selected.size > 0 && (
        <div className="flex items-center justify-between gap-3 px-4 py-3 rounded-xl"
          ...
          <span className="text-[13px] text-[#EDF0F5] font-medium">
            <span style={{ color: '#FF6B6B', fontWeight: 700 }}>{selected.size}</span> project terpilih
          </span>
          <div className="flex items-center gap-2">
            <button onClick={() => setSelected(new Set())}
              className="px-3 py-2 rounded-lg text-[12px] font-medium"
              style={{ background: 'rgba(255,255,255,0.06)', color: '#A5AEC0', border: '1px solid rgba(255,255,255,0.1)' }}>
              Batal
            </button>
            <button onClick={handleBulkDelete} disabled={bulkDeleting}
              className="flex items-center gap-1.5 px-3.5 py-2 rounded-lg text-[12px] font-semibold"
              style={{ background: bulkDeleting ? 'rgba(255,107,107,0.4)' : '#FF6B6B', color: '#0C0F16', cursor: bulkDeleting ? 'wait' : 'pointer' }}>
      ...
      {filtered.length === 0 ? (
        <Card>
          <div className="p-16 text-center">
            <FolderKanban size={40} className="mx-auto text-[#6B7385] mb-3" />
            <div className="text-[#6B7385] text-sm">Tidak ada project ditemukan.</div>
            {canCreateProject(currentUser.role) && (
        ...
        <Card>
          <div className="hidden sm:grid gap-4 px-4 py-3 border-b border-[rgba(255,255,255,0.06)] items-center"
            ...
            {['Nama Project', 'Divisi', 'Status', 'Prioritas', 'Progress', 'Deadline', ...(canEdit ? [''] : [])].map((h, i) => (
              <div key={h || `col-${i}`} className="font-mono text-[10px] tracking-widest text-[#6B7385] uppercase">{h}</div>
            ))}
          </div>
            ...
            {filtered.map(p => {
              ...
              return (
                <div key={p.id} className="group">
                  {/* Desktop row */}
                  <div className="hidden sm:grid gap-4 px-4 py-3.5 items-center hover:bg-white/[0.02] transition-colors cursor-pointer"
                    style={{ gridTemplateColumns: gridCols, background: selected.has(p.id) ? 'rgba(255,106,26,0.06)' : expanded ? 'rgba(255,255,255,0.02)' : undefined }}
                    ...
                    <div className="flex items-center gap-2 min-w-0">
                      <ChevronRight size={14} className="text-[#6B7385] flex-shrink-0 transition-transform"
                        style={{ transform: expanded ? 'rotate(90deg)' : 'none' }} />
                      <div className="min-w-0">
                        <div className="text-[13.5px] text-[#EDF0F5] font-medium group-hover:text-white leading-snug truncate">{p.name}</div>
                        <div className="font-mono text-[10px] text-[#6B7385] mt-0.5 flex items-center gap-2">
                          <span>{p.projectCode}</span>
                          {stats.total > 0 && (
                            <span className="inline-flex items-center gap-1" style={{ color: stats.done === stats.total ? '#3FD08A' : '#6B7385' }}>
                    ...
                    <div className="text-[12px] text-[#A5AEC0] truncate">{p.divisionName ?? '—'}</div>
                    ...
                    <div className="font-mono text-[11px] text-[#A5AEC0]">
                      {formatDate(p.deadline, { day: 'numeric', month: 'short', year: 'numeric' })}
                    </div>
                    {canEdit && (
                      <div className="flex gap-1" onClick={e => e.stopPropagation()}>
                        <Link href={`/projects/${p.id}/edit`}
                          style={{ background: 'rgba(255,255,255,0.06)', border: 'none', borderRadius: '7px', padding: '5px 7px', cursor: 'pointer', color: '#A5AEC0', display: 'flex', alignItems: 'center' }}>
                          <Pencil size={12} />
                        </Link>
                        <button onClick={e => handleDelete(e, p.id, p.status)}
                          ...
                          style={{ background: 'rgba(255,107,107,0.08)', border: 'none', borderRadius: '7px', padding: '5px 7px', cursor: 'pointer', color: (!NON_DELETEABLE.includes(p.status) || currentUser.role === 'super_admin') ? '#FF6B6B' : '#4a5160', display: 'flex', alignItems: 'center' }}>
                          <Trash2 size={12} />
                        </button>
                      </div>
                    )}
                  ...
                  {/* Mobile card */}
                  <div className="sm:hidden flex flex-col gap-3 p-4 hover:bg-white/[0.02] transition-colors cursor-pointer"
                    ...
                    <div className="flex items-start justify-between gap-2">
                      <div className="flex items-start gap-2.5">
                        ...
                        <div>
                          <div className="text-[13.5px] text-[#EDF0F5] font-medium">{p.name}</div>
                          <div className="font-mono text-[10px] text-[#6B7385] mt-0.5 flex items-center gap-2">
                            <span>{p.projectCode}</span>
                            {stats.total > 0 && (
                              <span className="inline-flex items-center gap-1" style={{ color: stats.done === stats.total ? '#3FD08A' : '#6B7385' }}>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2" onClick={e => e.stopPropagation()}>
                        {canEdit && (
                          <>
                            <Link href={`/projects/${p.id}/edit`} style={{ color: '#A5AEC0' }}><Pencil size={13} /></Link>
                            <button onClick={e => handleDelete(e, p.id, p.status)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: (!NON_DELETEABLE.includes(p.status) || currentUser.role === 'super_admin') ? '#FF6B6B' : '#4a5160', display: 'flex' }}><Trash2 size={13} /></button>
                          </>
                        )}
                        <span className="font-mono text-[11px] text-[#6B7385]">{formatDate(p.deadline, { day: 'numeric', month: 'short' })}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  {/* Expanded task list */}
                  {expanded && (
                    <div className="px-4 sm:px-12 pb-4 pt-1" style={{ background: 'rgba(255,255,255,0.015)' }}>
                      <div className="flex items-center justify-between mb-2">
                        <span className="font-mono text-[10px] tracking-widest text-[#6B7385] uppercase">
                      ...
                      {p.tasks.length === 0 ? (
                        <div className="text-[12px] text-[#6B7385] py-3 text-center rounded-lg" style={{ border: '1px dashed rgba(255,255,255,0.08)' }}>
                        ...
                        <div className="space-y-1.5">
                          {p.tasks.map(t => {
                            ...
                            return (
                              <div key={t.id} className="flex items-center gap-2.5 px-3 py-2 rounded-lg" style={{ background: 'rgba(255,255,255,0.025)' }}>
                                <button
                                  ...
                                  {t.checked && <Check size={11} strokeWidth={3} style={{ color: '#0C0F16' }} />}
                                </button>
                                <span className="flex-1 text-[12.5px] text-[#EDF0F5] truncate"
                                  style={{ textDecoration: t.checked ? 'line-through' : 'none', opacity: t.checked ? 0.55 : 1 }}>
                                  {t.name}
                                </span>
                                {t.assignees && <span className="text-[11px] text-[#6B7385] hidden sm:inline truncate max-w-[160px]">{t.assignees}</span>}
