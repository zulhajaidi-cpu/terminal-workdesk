...
export function KudosPanel({ status, teammates }: { status: KudosStatus; teammates: Teammate[] }) {
  ...
  const input: React.CSSProperties = {
    background: '#10141d', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 9, padding: '9px 11px',
    color: '#EDF0F5', fontSize: 13, width: '100%', fontFamily: "'Space Grotesk',sans-serif",
  }

  return (
    <div style={{ background: '#161a23', border: '1px solid rgba(255,106,138,0.18)', borderRadius: 18, padding: 18 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 2 }}>
        <Heart size={15} color="#FF6A8A" />
        <span style={{ fontFamily: "'Space Grotesk',sans-serif", fontWeight: 700, fontSize: 14, color: '#EDF0F5' }}>Kudos</span>
        <span style={{ marginLeft: 'auto', fontSize: 11, fontWeight: 800, color: noQuota ? '#6B7385' : '#FF8FA8', background: noQuota ? 'rgba(255,255,255,0.05)' : 'rgba(255,106,138,0.12)', padding: '3px 9px', borderRadius: 100 }}>
          {status.remaining}/{status.cap} tersisa
        </span>
      </div>
      <p style={{ fontSize: 11, color: '#6B7385', marginBottom: 14 }}>Apresiasi rekanmu — beri +{status.pointsPerKudos} EXP 💌</p>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 14 }}>
          ...
          <button onClick={send} disabled={!recipientId || busy}
            style={{
              ...
              background: !recipientId ? 'rgba(255,255,255,0.06)' : 'linear-gradient(90deg,#FF6A8A,#FF8FA8)',
              color: !recipientId ? '#6B7385' : '#fff', opacity: busy ? 0.6 : 1,
      ...
      {(status.received.length > 0 || status.given.length > 0) && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {status.received.length > 0 && (
            <KudosFeed label="Diterima" icon={<ArrowDownLeft size={12} color="#3FD08A" />} items={status.received} positive />
          )}
          {status.given.length > 0 && (
            <KudosFeed label="Diberikan" icon={<ArrowUpRight size={12} color="#FF8FA8" />} items={status.given} />
          )}
        </div>
      )}
    </div>
...
function KudosFeed({ label, icon, items, positive }: { label: string; icon: React.ReactNode; items: KudosFeedItem[]; positive?: boolean }) {
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 10.5, color: '#A5AEC0', fontFamily: "'IBM Plex Mono',monospace", textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 8 }}>
        {icon} {label}
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
        {items.map(k => (
          <div key={k.id} style={{ display: 'flex', alignItems: 'center', gap: 9, padding: '7px 9px', borderRadius: 10, background: 'rgba(255,255,255,0.02)' }}>
            <Avatar name={k.otherName} imageUrl={k.otherAvatar ?? undefined} size="sm" />
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 12, color: '#EDF0F5', fontFamily: "'Space Grotesk',sans-serif", whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{k.otherName}</div>
              {k.message && <div style={{ fontSize: 10.5, color: '#6B7385', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{k.message}</div>}
            </div>
            <div style={{ flexShrink: 0, textAlign: 'right' }}>
              {positive && <span style={{ fontSize: 12, fontWeight: 800, color: '#3FD08A', fontFamily: "'Space Grotesk',sans-serif" }}>+{k.points}</span>}
              <div style={{ fontSize: 9.5, color: '#4a5160', fontFamily: "'IBM Plex Mono',monospace" }}>{fmtDate(k.createdAt)}</div>
            </div>
          </div>
        ))}
      </div>
