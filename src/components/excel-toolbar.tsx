...
export function ExcelToolbar({ type, label }: { type: ExcelEntity; label: string }) {
  ...
  const btn: React.CSSProperties = {
    ...
    border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.04)', color: '#A5AEC0',
    whiteSpace: 'nowrap',
  }
    ...
    <>
      <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
        <button onClick={() => download('template')} style={btn} title="Unduh template kolom kosong">
          <FileSpreadsheet size={14} style={{ color: '#4A9EFF' }} /> Template
        </button>
        <button onClick={() => download('export')} style={btn} title="Download semua data sebagai Excel">
          <FileDown size={14} style={{ color: '#3FD08A' }} /> Export
        </button>
      ...
      {(result || errMsg) && (
        <div className="fixed inset-0 z-50 overflow-y-auto flex items-start justify-center p-4 py-8"
          ...
          <div className="w-full max-w-md rounded-2xl" style={{ background: '#10141d', border: '1px solid rgba(255,255,255,0.12)' }}
            ...
            <div style={{ padding: '18px 22px', borderBottom: '1px solid rgba(255,255,255,0.07)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <h3 className="font-grotesk font-bold text-[16px] text-[#EDF0F5]">Hasil Import {label}</h3>
              <button onClick={() => { setResult(null); setErrMsg(null) }} style={{ background: 'rgba(255,255,255,0.06)', border: 'none', borderRadius: '8px', padding: '6px', cursor: 'pointer', color: '#6B7385', display: 'flex' }}><X size={15} /></button>
            </div>
              ...
              {errMsg ? (
                ...
                <>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '16px' }}>
                    <CheckCircle2 size={22} style={{ color: '#3FD08A' }} />
                    <span className="text-[#EDF0F5] font-semibold text-[15px]">Import selesai</span>
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '8px', marginBottom: result.errors.length ? '16px' : 0 }}>
                    <Stat label="Ditambah" val={result.inserted} color="#3FD08A" />
                    <Stat label="Diperbarui" val={result.updated} color="#4A9EFF" />
                    <Stat label="Dilewati" val={result.skipped} color={result.skipped ? '#F59E0B' : '#6B7385'} />
                  </div>
                  {result.errors.length > 0 && (
                    <div>
                      <p style={{ fontSize: '11px', color: '#6B7385', fontFamily: "'IBM Plex Mono', monospace", textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '8px' }}>Baris bermasalah</p>
                      <div style={{ maxHeight: '180px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '5px' }}>
                        {result.errors.map((er, i) => (
                          <div key={i} style={{ fontSize: '12px', color: '#FCA5A5', background: 'rgba(239,68,68,0.08)', borderRadius: '7px', padding: '6px 10px' }}>
...
function Stat({ label, val, color }: { label: string; val: number; color: string }) {
  return (
    <div style={{ background: 'rgba(255,255,255,0.03)', borderRadius: '10px', padding: '12px', textAlign: 'center' }}>
      ...
      <p style={{ fontSize: '10px', color: '#6B7385', textTransform: 'uppercase', letterSpacing: '0.05em', marginTop: '2px' }}>{label}</p>
    </div>
  )
}
